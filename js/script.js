...
(function () {
	var userAgent = navigator.userAgent.toLowerCase(),
		...
		plugins = {
			...
			progressCircle2: document.querySelectorAll('.progress-circle2'),
	...
	$window.on('load', function () {
		...
		if (plugins.progressCircle2) {
			for (var i = 0; i < plugins.progressCircle2.length; i++) {
				var
					container = plugins.progressCircle2[i],
					counter = aCounter({
						node: container.querySelector('.progress-circle-counter2'),
						duration: 500,
						onUpdate: function (value) {
							this.custom.bar.render(value * 1.6);
						}
					});
				...
				counter.custom = {
					container: container,
					bar: aProgressCircle({ node: container.querySelector('.progress-circle-bar2') }),
					onScroll: (function () {
						if (Util.inViewport(this.custom.container) && !this.custom.container.classList.contains('animated')) {
							this.run();
							this.custom.container.classList.add('animated');
			...
			var regularConstraintsMessages = [
				{
					...
					newMessage: "Por favor, preencha o campo."
				},
				{
					type: regula.Constraint.Email,
					newMessage: "O e-mail que você usou não é válidos"
				},
				{
